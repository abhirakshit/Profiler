...
//            console.log("Focus IN has class")
          $el.removeClass("inline-placeholder");
          $el.css({"font-style" : oldFontStyle});
        }
      }).focusout(function () {
//            console.log("Focus OUT ---" + hasPlaceholder + "--" + !$el.html())
...
//        });
    ...
    $el.blur(function () {
      ...
      if (oldVal !== newVal && newVal !== "") {
          if (options.onSave) {
              var newAttribAndVal = options.onSave(newVal);
              // Check if valid value then save
              if (newAttribAndVal && newAttribAndVal.value !== "") {
                saveModel(newAttribAndVal.attribute, newAttribAndVal.value);
              } else { //On error
                // flash error msg
                flashMark("remove");
                //Re-enter previous val
                $el.html(oldVal);
              }
              return;
          }

        saveModel(attribute, newVal);
...
//        }});
       ...
       * @param newVal
       */
    function saveModel(modelAttrib, newValue) {
//          model.serverAttrs = [modelAttrib];
          console.log("Save value: " + newValue);
          model.save(modelAttrib, newValue, {
              wait: true,
              patch: true,
